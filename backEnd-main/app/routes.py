...
n jsonify({"message": "Student assigned to bus destination successfully"})
turn jsonify({"message": "Student assigned to bus destination successfully"})

@routes.route('/galdef get_terms():
    terms = Term.query.all()
    return jsonify([{
        'id': term.id,
        'name': term.name,
...
def assign_student_to_bus():
    ...
    return jsonify({def get_terms():
    terms = Term.query.all()
    return jsonify([{
        'id': termdef get_terms():
    terms = Term.query.all()
    return jsonify([{
        'id': term.id,
        'name': term.name,
...
def assign_student_to_bus():
    ...
    return jsonify({"messdef get_terms():
    terms = Term.query.all()
    return jsonify([{
        'id': term.id,
        'name': term.name,
...
def assign_student_to_bus():
    ...
    return jsonify({"message": "Student assigned to bus destination successfully"})
age": "Student assigned to bus destination successfully"})
"message": "Student assigned to bus destination successfully"})
tinations.append(destination)
    db.session.commit()

    return jsonify({"message": "Student assigned to bus destination successfully"})
rn jsonify({"error": "Student or Bus Destination not found"}), 404

    # Check if the def get_terms():
    terms = Term.query.all()
    return jsonify([{
        'id': term.id,
        'name': term.name,
...
def assign_student_to_bus():
    ...
    return jsonify({"message": "Student assigned to bus destination successfully"})
student is already assigned to this destination
    if destination in student.bus_destinations:
        return jsonify({"message": "Student is already assigned to this bus destination"})
